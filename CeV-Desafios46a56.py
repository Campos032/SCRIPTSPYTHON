...
# Desafio 49 Faça a tabuada utiliazando do laço for
...
# Dessa progressão
# Formula geral PA , an = a1 + (n – 1)r   a1 = primeiro termo da PA   n=a posição desejada do termo  r=a razão de 2 em 2
def pa():
...
def palin():
    ...
    reversestr = ''
    for letras in reversed(frase_sem_espaços):
        reversestr += letras
    if reversestr == frase_sem_espaços:
...
def idade():
    ...
    for i in range(0, 7):
        ...
        if i >= 18:
            idade.append(i)
    print(f'{idade.__len__()} pessoas são maiores de idade e {7 - idade.__len__()} são menores de idade!')
